...
def run(parser, args):
    ...
    for page in range(pages):
        logger.info(
            "%i of %i - %f%%" % (page * N, total, round(100 * (page * N) / total, 2))
        )
        for variant in (
            Variant.snps(id__in=_variant_ids).order_by("start").skip(N * page).limit(N)
        ):
            ...
            for i, ref in enumerate(variant_panel.refs):
                sys.stdout.write(
                    ...
                    % (
                        ...
                        variant.var_name[:100],
                        len(variant_panel.alts),
                        variant_panel.variant.reference.id,
                        i,
                    )
                )
                sys.stdout.write("%s\n" % ref)
