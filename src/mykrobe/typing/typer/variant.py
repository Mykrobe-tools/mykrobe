...
class VariantTyper(Typer):
    def __init__(
        ...
        super(VariantTyper, self).__init__(
            expected_depths,
            contamination_depths,
            error_rate,
    ...
    def _type_variant_probe_coverages(self, variant_probe_coverage, variant=None):
        ...
        if gt == "-/-" and not self.ignore_filtered:
            ...
            "LOW_PERCENT_COVERAGE" in self.filters
            and variant_probe_coverage.alternate_percent_coverage < 100
            and variant_probe_coverage.reference_percent_coverage < 100
        ):
...
class GenotypeModel(object):
    def __init__(
        self, expected_depths, contamination_depths, error_rate, minor_freq, kmer_size
    ):
...
class KmerCountGenotypeModel(GenotypeModel):
    ...
    def het_lik(self, variant_probe_coverage):
        ...
        if (
            ...
            variant_probe_coverage.alternate_percent_coverage < 100
            or variant_probe_coverage.reference_percent_coverage < 100
        ):
            return MIN_LLK
        else:
            het_liks = []
...
class DepthCoverageGenotypeModel(GenotypeModel):
    ...
    def hom_ref_lik(self, variant_probe_coverage):
        if (
            variant_probe_coverage.reference_percent_coverage
            < 100 * percent_coverage_from_expected_coverage(max(self.expected_depths))
        ):
            return MIN_LLK
        else:
            hom_ref_likes = []
