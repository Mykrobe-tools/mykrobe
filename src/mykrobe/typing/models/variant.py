...
class VariantProbeCoverage(object):
    def __init__(
        self, reference_coverages, alternate_coverages, var_name=None, params={}
    ):
    ...
    def _choose_best_coverage(self, coverages):
        coverages.sort(key=lambda x: x.k_count, reverse=True)
    ...
    def _choose_best_reference_coverage(self):
        best_reference_coverage = self._choose_best_coverage(self.reference_coverages)
        return best_reference_coverage

    @property
    def coverage_dict(self):
        return {
            "reference": self.best_reference_coverage.coverage_dict,
