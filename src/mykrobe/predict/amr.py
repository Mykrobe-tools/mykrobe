...
class BasePredictor(object):
    ...
    def _create_initial_resistance_prediction(self):
        self.result = MykrobePredictorSusceptibilityResult(
            dict((k, {"predict": "N"}) for k in self.drugs)
        )
    ...
    def predict_antibiogram(self):
        ...
        for name, gene in self.called_genes.items():
            if isinstance(gene, list):
                if len(gene) > 1:
                    logger.warning("Ambigious gene call. Continuing regardless. ")
                gene = gene[0]
    ...
    def _update_resistance_prediction(self, allele_name, variant_or_gene):
        ...
        for name in variant_or_gene_names:
            ...
            for drug in drugs:
                current_resistance_prediction = self.resistance_predictions[drug][
                    "predict"
                ]
    ...
    def _resistance_prediction(self, variant_or_gene, names):
        ...
        if sum(variant_or_gene.get("genotype")) == 2:
            if (
                is_filtered(variant_or_gene) and self.ignore_filtered
            ) or depth_on_alternate(variant_or_gene) < self.depth_threshold:
            ...
            if (
                ...
                self._coverage_greater_than_threshold(variant_or_gene, names)
                and not self.ignore_minor_calls
            ):
                __resistance_prediction = "r"
            else:
                __resistance_prediction = "S"
    ...
    def _coverage_greater_than_threshold(self, variant_or_gene, names):
        ...
        for name in names:
            if name in self._cn_threshold:
                coveage_threshold = self._cn_threshold.get(name, DEFAULT_MIN_VARIANT_CN)
...
class TBPredictor(BasePredictor):
    ...
    def default_variant_to_resistance_drug(self):
        ...
        return os.path.join(self.data_dir, "variant_to_resistance_drug.json")
    ...
    def __init__(
        ...
        super(TBPredictor, self).__init__(
            variant_calls,
            called_genes,
            base_json,
...
class StaphPredictor(BasePredictor):
    ...
    def default_variant_to_resistance_drug(self):
        self.data_dir = os.path.abspath(
            os.path.join(os.path.dirname(__file__), "../data/predict/staph/")
        )
        return os.path.join(self.data_dir, "variant_to_resistance_drug.json")
    ...
    def __init__(
        ...
        super(StaphPredictor, self).__init__(
            variant_calls,
            called_genes,
            base_json,
            depth_threshold=depth_threshold,
